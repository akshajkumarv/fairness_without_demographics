...
from jtt_data import WaterbirdsDataset
...
for STAGE_1_MODEL in STAGE_1_EPOCHS:  
    from jtt_data import train_split, train_waterbirds_loader, train_landbirds_loader
    ...
    df_waterbirds  = pd.DataFrame()
    with torch.no_grad():
        for i, (features, names, attributes, targets) in enumerate(train_waterbirds_loader):
            ...
            df_waterbirds = df_waterbirds.append(train_split.loc[train_split['img_filename'].isin(names)], ignore_index=True)
    
    df_landbirds  = pd.DataFrame()    
    with torch.no_grad():
        for i, (features, names, attributes, targets) in enumerate(train_landbirds_loader):
            features = features.to(DEVICE).float()
            targets = targets.to(DEVICE).unsqueeze(1).float()
            logits = model(features)
            probas = torch.sigmoid(logits)
            ...
            incorrect_predictions = list(incorrect_predictions.to('cpu').numpy())
            names = [names[i] for i in incorrect_predictions]
            df_landbirds = df_landbirds.append(train_split.loc[train_split['img_filename'].isin(names)], ignore_index=True)
    
    # Upsampling 
    for i in range(UPSAMPLE):
        train_split = train_split.append(df_waterbirds, ignore_index=True)
        train_split = train_split.append(df_landbirds, ignore_index=True)
